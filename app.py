...
app = Flask(__name__)
app.secret_key = "valami_sajat_secret"  # session miatt
...
def simulate_strategy(df, mode):
    ...
    for i in range(1,len(df)):
        ...
        if mode=="stock_only":
            ...
            ratio = (curr["Bond close"]/prev["Bond close"] if prev["Signal"]=="DEFENSIVE"
                     else curr["Stock close"]/prev["Stock close"])
        elif mode=="stock_gold":
            ratio = (curr["Gold close"]/prev["Gold close"] if prev["Signal"]=="DEFENSIVE"
                     else curr["Stock close"]/prev["Stock close"])
        capital *= ratio
...
def get_fx_rates():
    """Lekéri az EUR és USD árfolyamot HUF-hoz a Yahoo Finance-ről."""
    try:
        eur = yf.download("EURHUF=X", period="1d", interval="1d", progress=False, threads=False)
        usd = yf.download("USDHUF=X", period="1d", interval="1d", progress=False, threads=False)
        eur_rate = round(eur["Close"].dropna().iloc[-1],2)
        usd_rate = round(usd["Close"].dropna().iloc[-1],2)
...
def index():
    ...
    if request.method=="POST":
        ...
        if not errors:
            # PRG: mentjük sessionbe a form adatokat
            session["form_data"] = {"stock": stock, "bond": bond, "gold": gold, "ma": ma}
            return redirect(url_for("index"))

        # Ha hibák vannak, nem redirectelünk, csak visszaadjuk
        session["form_data"] = {"stock": stock, "bond": bond, "gold": gold, "ma": ma}
    ...
    # Adatok feldolgozása csak GET-ben, ha van form_data
    if form_data and not errors:
        try:
            ...
            eur_rate, usd_rate = get_fx_rates()
